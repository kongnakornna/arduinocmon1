...
IPAddress local_ip(172, 25, 99, 58);
IPAddress subnet(255, 255, 255, 240);
IPAddress gateway(172, 25, 99, 62);
IPAddress dns(8,8,8,8);  // dnsIP
...
const char* dataTopic = "AIRCOM1/DATA";
const char* controlTopic = "AIRCOM1/CONTROL"; 
...
PCF8574 RelayOutput(0x24);  // relay
...
void setup(){
    ...
    xTaskCreatePinnedToCore(
      loop_T0, "Task0", 5000, NULL, 0, &Task0, 0);
    ...
    mqttClient.setServer(mqttServer, mqttPort);  // ใช้ตัวแปร
    ...
    mqttClient.setServer(mqttServer, mqttPort); // FIXED: use variables
    mqttClient.setCallback(mqttCallback);
...
void reconnect() {
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect("AIRCOM1")) {
      Serial.println("connected");
      mqttClient.subscribe("AIRCOM1/CONTROL");
    } else {
...
void Control_relay(int number,int state){
  RelayOutput.write(number, state);
}
...
void Ck_Error_code() {
  if(status_temp_alarm == 0) Serial.println("High Temp"); else Serial.println("Temp normal");      
  if(dataFireAlarmInput == 0) Serial.println("Fire Alarm"); else Serial.println("Fire normal");
  if(dataHumiAlarmInput == 0) Serial.println("Humi Alarm"); else Serial.println("Humi normal");
  if(dataAir1AlarmInput == 0) Serial.println("Air1 Alarm"); else Serial.println("Air1 normal");
  if(dataAir2AlarmInput == 0) Serial.println("Air2 Alarm"); else Serial.println("Air2 normal");
}
...
void Read_temperature(){
  ...
  if(currentTemp > different_warning){ status_temp_alarm=0; TempAlarmOFF = 0; } else { status_temp_alarm=1; }
}

void SendDataMQTT() {
  char tempStr[6];         dtostrf(currentTemp, 4, 1, tempStr);
  char warningTempStr[6];  dtostrf(warningTemp, 4, 1, warningTempStr);
  char TempRecoveryStr[6]; dtostrf(TempRecovery, 4, 1, TempRecoveryStr);
  char periodStr[6];       dtostrf(period, 4, 1, periodStr);
  char percentStr[6];      dtostrf(percent, 4, 1, percentStr);
  char msgBuffer;
  snprintf(
    msgBuffer, sizeof(msgBuffer),
    "%s,%s,%s,%s,%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
    tempStr, warningTempStr, TempRecoveryStr, periodStr, percentStr,
    dataFireAlarmInput, dataHumiAlarmInput, dataAir2AlarmInput,
    dataAir1AlarmInput, status_temp_alarm, MODE, state_AIR1, state_AIR2,
    TempAlarmOFF, dataUPS1AlarmInput, dataUPS2AlarmInput, dataHSSDAlarmInput, dataWaterleakAlarmInput
  );
  Serial.println(msgBuffer);

  if (mqttClient.publish("AIRCOM1/DATA", msgBuffer)) {
    Serial.println("Sent:AIRCOM1/DATA ");
  } else {
