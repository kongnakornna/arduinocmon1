...
String SN = "AIRCOM1";  // Serial Number
...
void removeSpaces(char* str) {
  // สร้างตัวชี้สองตัว ตัวหนึ่งสำหรับอ่าน (readPtr) และอีกตัวสำหรับเขียน (writePtr)
  char* readPtr = str;
  char* writePtr = str;

  // วนลูปจนกว่าจะเจอจุดสิ้นสุดของสตริง (\0)
  while (*readPtr) {
    // ถ้าตัวอักษรที่อ่านไม่ใช่ช่องว่าง
    if (*readPtr != ' ') {
      // ให้คัดลอกตัวอักษรนั้นไปยังตำแหน่งที่จะเขียน
      *writePtr = *readPtr;
      // แล้วเลื่อนตำแหน่งเขียนไปข้างหน้า
      writePtr++;
    }
    // เลื่อนตำแหน่งอ่านไปข้างหน้าเสมอ
    readPtr++;
  }
  
  // ปิดท้ายสตริงที่แก้ไขแล้วด้วย null terminator
  *writePtr = '\0';
}
...
void SendDataMQTT() {
  ...
  // เรียกใช้ฟังก์ชันเพื่อลบช่องว่าง
  removeSpaces(msgBuffer);
